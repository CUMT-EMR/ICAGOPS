...
class Model(nn.Module):
    def __init__(self):
        ...
        self.lstm = nn.LSTM(input_size=1024, hidden_size=seq_len,num_layers=2,batch_first=True,bidirectional=False).to(device)
        self.dropout = nn.Dropout(0.3)
        self.linear2 = nn.Linear(seq_len,1).to(device)
    def forward(self,data):
        data = torch.unsqueeze(data,dim=1)
        rnndata, (hn, cn) =self.lstm(data)
        x = self.dropout(rnndata[:, -1, :])
        tgt = self.linear2(x)
...
def train(trans_dataloader,test_validation,iter):
    ...
    data_result_write = pd.ExcelWriter("./trainlossAE/{}_{}_result_loss_LSTM.xlsx".format(model_name,iter))
    data_testvali_result_write = pd.ExcelWriter("./trainlossAE/{}_{}_result_vali_loss_LSTM.xlsx".format(model_name,iter))
    data_trainvail_result_write = pd.ExcelWriter("./trainlossAE/{}_{}_result_train_loss_LSTM.xlsx".format(model_name,iter))
    data_optostep_result_write = pd.ExcelWriter("./trainlossAE/{}_{}_result_train_opti_step_LSTM.xlsx".format(model_name,iter))
...
def prediction(test_loader,model,echop,name,iter):
    ...
    data_result_write = pd.ExcelWriter('./tmplstmAE/{}_{}_'.format(model_name,iter)+name+"_result_test_new_"+str(echop)+"_LSTM.xlsx")
    result.to_excel(data_result_write)
    data_result_write.close()
scaler = MinMaxScaler() 
def main():
