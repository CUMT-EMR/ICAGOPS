...
class Model(nn.Module):
    def __init__(self):
        ...
        self.linear1 = nn.Linear(1,d_model).to(device)
        self.cnn =  nn.Sequential(nn.Conv1d(in_channels=1,out_channels=5,kernel_size=5,padding=0),nn.ReLU(),nn.MaxPool1d(kernel_size=5)).to(device)
        self.linear2 = nn.Linear(204*5,1).to(device)
    def forward(self,data):
        data = torch.unsqueeze(data,dim=1)
        cnndata=self.cnn(data)
        size = data.shape[0]
        tgt = self.linear2(cnndata.reshape(size,-1))
...
def train(trans_dataloader,test_validation,iter):
    ...
    data_result_write = pd.ExcelWriter("./trainlossorigincnnAE/{}_result_loss_1000_tcn_wasi.xlsx".format(iter))
    data_testvali_result_write = pd.ExcelWriter("./trainlossorigincnnAE/{}_result_vali_loss_1000_tcn_wasi.xlsx".format(iter))
    data_trainvail_result_write = pd.ExcelWriter("./trainlossorigincnnAE/{}_result_train_loss_1000_tcn_wasi.xlsx".format(iter))
    data_optostep_result_write = pd.ExcelWriter("./trainlossorigincnnAE/{}_result_train_opti_step_1000_tcn_wasi.xlsx".format(iter))
...
def prediction(test_loader,model,echop,name,iter):
    ...
    data_result_write = pd.ExcelWriter('./tmporigincnnAE/{}_'.format(iter)+name+"_result_test_new_"+str(echop)+"_600_tcn_wasi.xlsx")
    result.to_excel(data_result_write)
    data_result_write.close()
scaler = MinMaxScaler() 
def main():
